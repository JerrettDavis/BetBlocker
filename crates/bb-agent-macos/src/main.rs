...
use dns_redirect::PfManager;

const AGENT_VERSION: &str = env!("CARGO_PKG_VERSION");

/// BetBlocker Agent for macOS -- gambling site blocking service.
#[derive(Parser, Debug)]
#[command(name = "bb-agent-macos", version, about)]
struct Cli {
    /// Path to the configuration directory.
    #[arg(long, default_value = "/Library/Application Support/BetBlocker")]
    config_dir: PathBuf,
...
async fn main() {
    ...
    tracing::info!(
        ...
        "BetBlocker Agent (macOS) starting"
    );
...
async fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Setup signal handling (SIGTERM + SIGINT on unix)
    #[cfg(unix)]
    {
    ...
    bb_shim_macos::platform::ensure_directories()
    ...
    // Build authenticated API client with mTLS
    ...
    // Install pf rules for DNS redirection
    let resolver_port = config.dns.listen_port;
    let mut pf_manager = PfManager::new(resolver_port);
    ...
    // Start pf rule verification loop
    let pf_shutdown_rx = shutdown_rx.clone();
    let pf_emitter = event_emitter.handle();
    let mut pf_shutdown = pf_shutdown_rx;
    let pf_task = tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                _ = ticker.tick() => {
                    match pf_manager.verify_and_repair() {
                        Ok(true) => {
                            pf_emitter.emit(bb_agent_core::events::AgentEvent::tamper_detected(
                                "pf",
                        ...
                        Err(e) => {
                            tracing::debug!(error = %e, "pf verification failed");
                        }
                    }
                }
                _ = pf_shutdown.changed() => {
                    break;
                }
            }
        }
    ...
    // Notify readiness (no-op on macOS, API parity with Linux)
